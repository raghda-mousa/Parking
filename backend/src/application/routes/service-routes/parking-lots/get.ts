...
router.get(
    ...
    async (req: Request, res: Response) => {
        ...
        const result = await parkingLotService.list(req.query.page as string, req.query.limit as string, req.query.searchKey as string);
...
router.get(
    ...
    async (req: Request, res: Response) => {
        const { id } = req.params
        const parkingLotService = new ParkingLotService();
        const result = await parkingLotService.getParkingLotById(id);
        if (result) {
            return ResponseService.sendSuccess(res, result, 'retrieved successfully')
        }
